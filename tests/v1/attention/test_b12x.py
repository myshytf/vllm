...
def test_b12x_attention_rejects_fp16_queries_with_fp8_kv(
    ...
    assert reason == "B12X currently requires bfloat16 queries"
...
def test_b12x_attention_rejects_float16_kv_dtype(
    ...
    assert reason == "B12X does not support float16 KV cache"


def test_b12x_attention_requires_optional_package(
    default_vllm_config,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(b12x, "get_b12x_paged_attention", lambda: None)
...
def test_b12x_attention_requires_supported_device_api(
    default_vllm_config,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    paged_attention = SimpleNamespace(is_supported=lambda: False)
    monkeypatch.setattr(
        b12x,
    ...
    assert reason == "B12X paged attention is not supported on the current device"

...
def test_b12x_moe_warmup_skips_unused_provider(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def fail_token_counts(**kwargs):
        pytest.fail("unused B12X MoE provider was warmed")

    monkeypatch.setattr(b12x_moe, "b12x_warmup_token_counts", fail_token_counts)

    assert b12x_moe.warmup_b12x_moe(torch.nn.Module(), max_tokens=128) == 0

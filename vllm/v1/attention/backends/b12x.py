...
class B12XPagedAttentionBackend(AttentionBackend):
    ...
    def supports_combination(
        ...
        if parallel_config.decode_context_parallel_size > 1:
            return "B12X does not yet support decode context parallelism"
        if parallel_config.prefill_context_parallel_size > 1:
            return "B12X does not yet support prefill context parallelism"
        return None
    ...
    def get_kv_cache_stride_order(
        ...
        if cache_layout != "NHD":
            raise ValueError(
                f"B12X requires NHD KV cache layout; got {cache_layout!r}."
            )
...
class B12XPagedAttentionImpl(AttentionImpl[B12XPagedMetadata]):
    ...
    def __init__(
        ...
        B12XPagedAttentionScratchCaps = paged_attention.Caps
        compile_paged_attention = paged_attention.compile
        plan_decode_graph_capacity = paged_attention.decode_graph_capacity
        plan_decode_graph_scratch_envelope = (
            paged_attention.decode_graph_scratch_envelope
        )
        plan_extend_graph_capacity = paged_attention.extend_graph_capacity
        plan_paged_attention_scratch = paged_attention.plan
        paged_attention_forward = paged_attention.run
        plan_verify_graph_capacity = paged_attention.verify_graph_capacity
    ...
    def _prepare_sinks(
        ...
        if sinks.device != device:
            raise RuntimeError("B12X sinks must be on the same CUDA device as query.")
        ...
        if _capture_alloc_forbidden():
            raise RuntimeError(
                "B12X would convert attention sinks during CUDA graph "

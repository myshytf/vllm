...
def warmup_b12x_moe(
    ...
    *,
    ...
    layer_map: dict[tuple[Any, ...], tuple[B12xExperts, torch.nn.Module]] = {}
    for module in model.modules():
        ...
        if signature is not None:
            layer_map.setdefault(signature, (fused_experts, routed_experts))
    if not layer_map:
        return 0

    candidates = b12x_warmup_token_counts(
        max_tokens=max_tokens,
        cudagraph_capture_sizes=token_counts,
    )
    warmed = 0
    for fused_experts, routed_experts in layer_map.values():
